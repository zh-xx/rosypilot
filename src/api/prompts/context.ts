...
// Also matches cursor immediately after the marker with no space yet (e.g. "-\uFFFF").
...
const INLINE_MATH_BLOCK_REGEX = /\$[\s\S]*?\$/;
const CODE_BLOCK_REGEX = /```(?<language>.*)[\s\S]*?```/;
const INLINE_CODE_BLOCK_REGEX = /`.*`/;
